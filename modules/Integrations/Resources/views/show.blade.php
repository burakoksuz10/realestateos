...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white">Entegrasyon Ayarları</h1>
            <p class="text-dark-400 mt-1">{{ is_array($integration) ? ($integration['name'] ?? $integration) : $integration }}</p>
        </div>
        <a href="{{ route('admin.integrations.index') }}" class="px-4 py-2 bg-dark-700 hover:bg-dark-600 text-white rounded-xl transition-colors flex items-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-white mb-6">Bağlantı Ayarları</h2>
                <form action="{{ route('admin.integrations.update', is_array($integration) ? $integration['id'] : $integration) }}" method="POST" class="space-y-4">
                    @csrf @method('PUT')
                    <div>
                        <label class="block text-sm font-medium text-dark-300 mb-2">API Anahtarı</label>
                        <input type="password" name="api_key" placeholder="••••••••••••••••"
                            class="w-full px-4 py-2.5 bg-dark-800 border border-dark-700 rounded-xl text-white placeholder-dark-500 focus:outline-none focus:ring-2 focus:ring-primary-500">
                        <p class="text-xs text-dark-500 mt-1">Mevcut anahtarı değiştirmek istiyorsanız girin.</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-dark-300 mb-2">API Gizli Anahtarı</label>
                        <input type="password" name="api_secret" placeholder="••••••••••••••••"
                            class="w-full px-4 py-2.5 bg-dark-800 border border-dark-700 rounded-xl text-white placeholder-dark-500 focus:outline-none focus:ring-2 focus:ring-primary-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-dark-300 mb-2">Webhook URL</label>
                        <input type="url" name="webhook_url" placeholder="https://example.com/webhook"
                            class="w-full px-4 py-2.5 bg-dark-800 border border-dark-700 rounded-xl text-white placeholder-dark-500 focus:outline-none focus:ring-2 focus:ring-primary-500">
                    </div>
                    <div class="pt-2">
                        <button type="submit" class="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-xl transition-colors">Ayarları Kaydet</button>
                    </div>
                </form>
            </div>

            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-white mb-4">Bağlantı Testi</h2>
                <p class="text-dark-400 text-sm mb-4">Entegrasyon bağlantısını test etmek için aşağıdaki butona tıklayın.</p>
                <button
                    onclick="testConnection()"
                    class="px-6 py-2.5 bg-dark-700 hover:bg-dark-600 text-white font-medium rounded-xl transition-colors flex items-center gap-2">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-sm font-medium text-dark-400 uppercase tracking-wider mb-4">Açıklama</h2>
                <p class="text-dark-300 text-sm">{{ is_array($integration) ? ($integration['description'] ?? 'Entegrasyon açıklaması bulunamadı.') : 'Entegrasyon detayları.' }}</p>
            </div>
        </div>
    </div>
</div>
