...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 dark:text-dark-400 mt-1">Rol detayları ve yetkileri</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.roles.edit', $role) }}" class="px-4 py-2 bg-gradient-to-r from-sky-400 to-blue-600 hover:from-sky-500 hover:to-blue-700 text-white rounded-xl transition-colors flex items-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                ...
                @empty
                <p class="text-gray-500 dark:text-dark-400">Bu role henüz yetki atanmamış.</p>
                @endforelse
            </div>

            <!-- Users with this role -->
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">Bu Roldeki Kullanıcılar</h2>
                ...
                <div class="flex items-center justify-between py-3 border-b border-gray-200 dark:border-dark-700/50 last:border-0">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 dark:text-dark-400 text-xs">{{ $user->email }}</p>
                        </div>
                    </div>
                    <a href="{{ route('admin.users.show', $user) }}" class="text-primary-400 hover:text-primary-300 text-sm">Görüntüle</a>
                </div>
                @empty
                <p class="text-gray-500 dark:text-dark-400 text-sm">Bu role atanmış kullanıcı yok.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Özet</h2>
                <div class="space-y-3">
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-dark-400 text-sm">Toplam Yetki</span>
                        <span class="text-white font-semibold">{{ $role->permissions->count() }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-dark-400 text-sm">Kullanıcı Sayısı</span>
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="px-2 py-0.5 text-xs rounded-full {{ in_array($role->name, ['super-admin','admin']) ? 'bg-purple-500/20 text-purple-400' : 'bg-primary-100 dark:bg-primary-500/20 text-primary-600 dark:text-primary-400' }}">
            ...
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6 space-y-3">
                <a href="{{ route('admin.roles.edit', $role) }}" class="w-full px-4 py-2.5 bg-gradient-to-r from-sky-400 to-blue-600 hover:from-sky-500 hover:to-blue-700 text-white font-medium rounded-xl transition-colors flex items-center justify-center">
                    Düzenle
                </a>
