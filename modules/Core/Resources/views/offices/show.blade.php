...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white">{{ $office->name }}</h1>
            <p class="text-dark-400 mt-1">{{ $office->city }}{{ $office->district ? ', ' . $office->district : '' }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.offices.edit', $office) }}" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl transition-colors flex items-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-dark-400 text-sm">Danışman</p>
            </div>
        </div>
        <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-dark-400 text-sm">Aktif İlan</p>
            </div>
        </div>
        <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-dark-400 text-sm">Takım</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-white mb-6">Danışmanlar</h2>
                ...
                <div class="flex items-center justify-between py-3 border-b border-dark-700/50 last:border-0">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-dark-400 text-xs">{{ $user->title ?? $user->email }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $user->is_active ? 'bg-green-500/20 text-green-400' : 'bg-dark-700 text-dark-400' }}">
                ...
                @empty
                <p class="text-dark-400 text-sm">Bu ofiste henüz danışman yok.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-white mb-4">Ofis Bilgileri</h2>
                <div class="space-y-3">
                    @if($office->code)
                    <div class="flex justify-between">
                        <span class="text-dark-400 text-sm">Kod</span>
                        <span class="text-white text-sm font-mono">{{ $office->code }}</span>
                    </div>
                    @endif
                    @if($office->phone)
                    <div class="flex justify-between">
                        <span class="text-dark-400 text-sm">Telefon</span>
                        <span class="text-white text-sm">{{ $office->phone }}</span>
                    </div>
                    @endif
                    @if($office->email)
                    <div class="flex justify-between">
                        <span class="text-dark-400 text-sm">E-posta</span>
                        <span class="text-white text-sm">{{ $office->email }}</span>
                    </div>
                    @endif
                    @if($office->address)
                    <div>
                        <span class="text-dark-400 text-sm block mb-1">Adres</span>
                        <span class="text-white text-sm">{{ $office->address }}, {{ $office->city }}</span>
                    </div>
                    @endif
                    <div class="flex justify-between">
                        <span class="text-dark-400 text-sm">Durum</span>
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $office->is_active ? 'bg-green-500/20 text-green-400' : 'bg-dark-700 text-dark-400' }}">
            ...
            @if($office->manager)
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-sm font-medium text-dark-400 uppercase tracking-wider mb-4">Ofis Yöneticisi</h2>
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center text-white text-sm font-semibold">
                    ...
                    <div>
                        ...
                        <p class="text-dark-400 text-xs">{{ $office->manager->email }}</p>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6 space-y-3">
                <a href="{{ route('admin.offices.edit', $office) }}" class="w-full px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-xl transition-colors flex items-center justify-center">
                    Düzenle
                </a>
