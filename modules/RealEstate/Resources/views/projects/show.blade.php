...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 dark:text-dark-400 mt-1">{{ $project->city }}{{ $project->district ? ', ' . $project->district : '' }}
        ...
        <div class="flex items-center gap-3">
            <form action="{{ route('admin.projects.toggle-featured', $project) }}" method="POST">
                @csrf
                <button type="submit" class="px-4 py-2 {{ $project->is_featured ? 'bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30' : 'bg-gray-100 dark:bg-dark-700 hover:bg-gray-200 dark:hover:bg-dark-600 text-gray-700 dark:text-white' }} rounded-xl transition-colors flex items-center">
            ...
            <a href="{{ route('admin.projects.edit', $project) }}" class="px-4 py-2 bg-gradient-to-r from-sky-400 to-blue-600 hover:from-sky-500 hover:to-blue-700 text-white rounded-xl transition-colors flex items-center">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                Düzenle
            </a>
            <a href="{{ route('admin.projects.index') }}" class="px-4 py-2 bg-gray-100 dark:bg-dark-700 hover:bg-gray-200 dark:hover:bg-dark-600 text-gray-700 dark:text-white rounded-xl transition-colors flex items-center">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-5">
            <p class="text-gray-500 dark:text-dark-400 text-sm">Toplam Daire</p>
            <p class="text-2xl font-bold text-white mt-1">{{ $project->total_units ?? '-' }}</p>
        </div>
        <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-5">
            <p class="text-gray-500 dark:text-dark-400 text-sm">Mevcut Daire</p>
            <p class="text-2xl font-bold text-green-400 mt-1">{{ $project->available_units ?? '-' }}</p>
        </div>
        <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-5">
            <p class="text-gray-500 dark:text-dark-400 text-sm">Min. Fiyat</p>
            <p class="text-2xl font-bold text-white mt-1">{{ $project->min_price ? '₺' . number_format($project->min_price, 0, ',', '.') : '-' }}</p>
        </div>
        <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-5">
            <p class="text-gray-500 dark:text-dark-400 text-sm">Maks. Fiyat</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-3">Açıklama</h2>
                <p class="text-gray-600 dark:text-dark-300 text-sm leading-relaxed">{{ $project->description }}</p>
            </div>
            @endif
            ...
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">İlanlar</h2>
                ...
                <div class="flex items-center justify-between py-3 border-b border-gray-200 dark:border-dark-700/50 last:border-0">
                    ...
                    <div class="text-right">
                        ...
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $listing->status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-200 dark:bg-dark-700 text-gray-500 dark:text-dark-400' }}">
                            {{ ucfirst($listing->status) }}
                        </span>
                    </div>
                </div>
                @empty
                <p class="text-gray-500 dark:text-dark-400 text-sm">Bu projeye ait ilan yok.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-sm font-medium text-gray-500 dark:text-dark-400 uppercase tracking-wider mb-4">Proje Bilgileri</h2>
                <div class="space-y-2">
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-dark-400 text-sm">Durum</span>
                        <span class="px-2 py-0.5 text-xs rounded-full
                            {{ $project->status === 'completed' ? 'bg-green-500/20 text-green-400' :
                               ($project->status === 'under_construction' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-primary-100 dark:bg-primary-500/20 text-primary-600 dark:text-primary-400') }}">
                            {{ ['planning'=>'Planlama','under_construction'=>'İnşaat','completed'=>'Tamamlandı'][$project->status] ?? $project->status }}
                        </span>
                    </div>
                    @if($project->developer)
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-dark-400 text-sm">Geliştirici</span>
                        <span class="text-white text-sm">{{ $project->developer }}</span>
                    </div>
                    @endif
                    @if($project->delivery_date ?? $project->estimated_completion)
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-dark-400 text-sm">Teslim Tarihi</span>
                        <span class="text-white text-sm">{{ \Carbon\Carbon::parse($project->delivery_date ?? $project->estimated_completion)->format('d.m.Y') }}</span>
                    </div>
                    @endif
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-dark-400 text-sm">Konum</span>
                        <span class="text-white text-sm">{{ $project->city }}, {{ $project->district }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-dark-400 text-sm">Aktif İlan</span>
            ...
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6 space-y-3">
                <a href="{{ route('admin.projects.edit', $project) }}" class="block w-full px-4 py-2.5 bg-gradient-to-r from-sky-400 to-blue-600 hover:from-sky-500 hover:to-blue-700 text-white font-medium rounded-xl transition-colors text-center">Düzenle</a>
