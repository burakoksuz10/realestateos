...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 dark:text-dark-400 mt-1">{{ $listing->reference_no }}</p>
        </div>
        <div class="flex items-center space-x-3">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-gray-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Fotoğraflar</h2>
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    @forelse($listing->getMedia('photos') as $photo)
                        <div class="aspect-video bg-gray-200 dark:bg-dark-700 rounded-lg overflow-hidden">
            ...
            <div class="bg-gray-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Açıklama</h2>
                    <button type="button"
                            onclick="generateDescription({{ $listing->id }}, this)"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white transition-colors">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
                        <span data-description-label>AI ile Üret</span>
                    </button>
                </div>
            ...
            <div class="bg-gray-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
                ...
                <div class="space-y-2">
                    <p class="text-gray-600 dark:text-dark-300"><i class="fas fa-map-marker-alt text-primary-400 mr-2"></i>{{ $listing->city }}, {{ $listing->district }}</p>
                    @if($listing->neighborhood)
                        <p class="text-gray-500 dark:text-dark-400 ml-6">{{ $listing->neighborhood }}</p>
                    @endif
                    @if($listing->address)
                        <p class="text-gray-500 dark:text-dark-400 ml-6">{{ $listing->address }}</p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-gradient-to-br from-primary-900/50 to-dark-800 rounded-xl border border-primary-500/30 p-6">
                <p class="text-gray-500 dark:text-dark-400 text-sm mb-1">Fiyat</p>
                <p class="text-3xl font-bold text-white">
                    {{ number_format($listing->price, 0, ',', '.') }}
                    <span class="text-lg">{{ $listing->price_currency }}</span>
                </p>
                <p class="text-gray-500 dark:text-dark-400 text-sm mt-2">
            ...
            <!-- Danışman -->
            <div class="bg-gray-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Danışman</h3>
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 dark:text-dark-400 text-sm">{{ $listing->agent->email ?? '' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-dark-400">Görüntülenme</span>
                        <span class="text-white font-medium">{{ $listing->view_count ?? 0 }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-dark-400">Favorilere Eklenme</span>
                        <span class="text-white font-medium">{{ $listing->favorite_count ?? 0 }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-dark-400">Talep Sayısı</span>
            ...
            <!-- Aksiyonlar -->
            <div class="bg-gray-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Aksiyonlar</h3>
                <div class="space-y-2">
                    ...
                    <form action="{{ route('admin.listings.duplicate', $listing) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full px-4 py-2 bg-gray-200 dark:bg-dark-700 text-gray-600 dark:text-dark-300 rounded-lg hover:bg-dark-600 transition-colors">
            ...
            <!-- Tarihler -->
            <div class="bg-gray-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Tarihler</h3>
                <div class="space-y-2 text-sm">
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-dark-400">Oluşturulma</span>
                        <span class="text-white">{{ $listing->created_at->format('d.m.Y H:i') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-dark-400">Güncelleme</span>
                        <span class="text-white">{{ $listing->updated_at->format('d.m.Y H:i') }}</span>
                    </div>
                    @if($listing->published_at)
                        <div class="flex items-center justify-between">
                            <span class="text-gray-500 dark:text-dark-400">Yayınlanma</span>
    ...
    @if(isset($similarListings) && $similarListings->count() > 0)
        <div class="bg-gray-100 dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-dark-700 p-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Benzer İlanlar</h2>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                @foreach($similarListings as $similar)
                    <a href="{{ route('admin.listings.show', $similar) }}" class="bg-gray-200 dark:bg-dark-700 rounded-lg p-4 hover:bg-dark-600 transition-colors">
                        ...
                        <p class="text-gray-500 dark:text-dark-400 text-sm">{{ $similar->city }}, {{ $similar->district }}</p>
                    </a>
                @endforeach
            </div>
        </div>
