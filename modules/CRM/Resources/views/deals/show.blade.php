...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white">{{ $deal->title }}</h1>
            <p class="text-dark-400 mt-1">Satış #{{ $deal->id }}
        ...
        <div class="flex items-center gap-3">
            @if($deal->status === 'open')
            <a href="{{ route('admin.deals.edit', $deal) }}" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl transition-colors flex items-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-5">
            <p class="text-dark-400 text-sm mb-1">Satış Değeri</p>
            <p class="text-3xl font-bold text-white">₺{{ number_format($deal->value ?? 0, 0, ',', '.') }}</p>
        </div>
        <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-5">
            <p class="text-dark-400 text-sm mb-1">Olasılık</p>
            <p class="text-3xl font-bold text-white">%{{ $deal->probability ?? 0 }}</p>
        </div>
        <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-5">
            <p class="text-dark-400 text-sm mb-1">Komisyon</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-white mb-4">Aktivite Geçmişi</h2>
                ...
                @empty
                <p class="text-dark-400 text-sm">Henüz aktivite kaydı yok.</p>
                @endforelse
            </div>
            ...
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-white mb-4">Görevler</h2>
        ...
        <div class="space-y-6">
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-sm font-medium text-dark-400 uppercase tracking-wider mb-4">Satış Bilgileri</h2>
                <div class="space-y-2">
                    <div class="flex justify-between">
                        <span class="text-dark-400 text-sm">Pipeline</span>
                        <span class="text-white text-sm">{{ $deal->pipeline->name ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-dark-400 text-sm">Aşama</span>
            ...
            @if($deal->contact)
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-sm font-medium text-dark-400 uppercase tracking-wider mb-4">Müşteri</h2>
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-dark-400 text-xs">{{ $deal->contact->phone ?? $deal->contact->email ?? '-' }}</p>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6 space-y-3">
                ...
                <a href="{{ route('admin.deals.commission', $deal) }}" class="block w-full px-4 py-2.5 bg-dark-700 hover:bg-dark-600 text-white font-medium rounded-xl transition-colors text-center">
                    Komisyon Detayı
                </a>
            </div>
            @endif
...
<div id="wonModal" class="hidden fixed inset-0 bg-black/70 flex items-center justify-center z-50">
    <div class="bg-dark-900 border border-dark-700 rounded-2xl p-6 w-full max-w-md mx-4">
        <h3 class="text-lg font-semibold text-white mb-4">Kazanıldı İşaretle</h3>
        <form action="{{ route('admin.deals.mark-won', $deal) }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-dark-300 mb-2">Kazanma Sebebi</label>
                <textarea name="won_reason" rows="3" class="w-full px-4 py-2.5 bg-dark-800 border border-dark-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Kısa bir açıklama..."></textarea>
            </div>
            <div class="flex gap-3">
                ...
                <button type="button" onclick="document.getElementById('wonModal').classList.add('hidden')" class="flex-1 px-4 py-2.5 bg-dark-700 hover:bg-dark-600 text-white font-medium rounded-xl transition-colors">İptal</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="bg-dark-900 border border-dark-700 rounded-2xl p-6 w-full max-w-md mx-4">
        ...
        <form action="{{ route('admin.deals.mark-lost', $deal) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="lost_reason" required rows="3" class="w-full px-4 py-2.5 bg-dark-800 border border-dark-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="Sebep açıklayın..."></textarea>
            </div>
            <div class="flex gap-3">
