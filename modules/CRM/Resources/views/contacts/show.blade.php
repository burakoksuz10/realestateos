...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $contact->first_name }} {{ $contact->last_name }}</h1>
            <p class="text-gray-500 dark:text-dark-400 mt-1">{{ $contact->company_name ?? $contact->email ?? '-' }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.contacts.edit', $contact) }}" class="px-4 py-2 bg-gradient-to-r from-sky-400 to-blue-600 hover:from-sky-500 hover:to-blue-700 text-white rounded-xl transition-colors flex items-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Talepler</h2>
                ...
                <div class="flex items-center justify-between py-3 border-b border-gray-200 dark:border-dark-700/50 last:border-0">
                    <div>
                        ...
                        <p class="text-gray-500 dark:text-dark-400 text-xs">{{ $lead->created_at->format('d.m.Y') }}</p>
                    </div>
                    <span class="px-2 py-0.5 text-xs rounded-full bg-primary-100 dark:bg-primary-500/20 text-primary-600 dark:text-primary-400">{{ ucfirst($lead->status) }}</span>
                </div>
                @empty
                <p class="text-gray-500 dark:text-dark-400 text-sm">Henüz talep kaydı yok.</p>
                @endforelse
            </div>

            <!-- Deals -->
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Fırsatlar</h2>
                ...
                <div class="flex items-center justify-between py-3 border-b border-gray-200 dark:border-dark-700/50 last:border-0">
                    <div>
                        ...
                        <p class="text-gray-500 dark:text-dark-400 text-xs">₺{{ number_format($deal->value ?? 0, 0, ',', '.') }}</p>
                    </div>
                ...
                @empty
                <p class="text-gray-500 dark:text-dark-400 text-sm">Henüz fırsat kaydı yok.</p>
                @endforelse
            </div>
            ...
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                ...
                <div class="flex gap-3 py-3 border-b border-gray-200 dark:border-dark-700/50 last:border-0">
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 dark:text-dark-400 text-xs">{{ $activity->created_at->diffForHumans() }}</p>
                    </div>
                </div>
                @empty
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $contact->status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-200 dark:bg-dark-700 text-gray-500 dark:text-dark-400' }}">
                ...
                <div class="space-y-2">
                    @if($contact->phone)
                    <div class="flex items-center gap-2 text-gray-600 dark:text-dark-300 text-sm">
                    ...
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-dark-400 text-sm">Kayıt Tarihi</span>
            ...
            <div class="bg-white dark:bg-dark-900 border border-gray-200 dark:border-dark-700/50 rounded-2xl p-6 space-y-3">
                <a href="{{ route('admin.contacts.edit', $contact) }}" class="block w-full px-4 py-2.5 bg-gradient-to-r from-sky-400 to-blue-600 hover:from-sky-500 hover:to-blue-700 text-white font-medium rounded-xl transition-colors text-center">Düzenle</a>
                <form action="{{ route('admin.contacts.toggle-status', $contact) }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full px-4 py-2.5 bg-gray-100 dark:bg-dark-700 hover:bg-gray-200 dark:hover:bg-dark-600 text-gray-700 dark:text-white font-medium rounded-xl transition-colors">
