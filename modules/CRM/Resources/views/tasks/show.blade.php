...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-dark-400 mt-1">Görev #{{ $task->id }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.tasks.edit', $task) }}" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl transition-colors flex items-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-white mb-4">Görev Detayları</h2>
                @if($task->description)
                <p class="text-dark-300 text-sm mb-4">{{ $task->description }}</p>
                @endif
                <div class="grid grid-cols-2 gap-4">
                    <div class="p-4 bg-dark-800/50 rounded-xl">
                        <p class="text-dark-400 text-xs mb-1">Tür</p>
                    ...
                    <div class="p-4 bg-dark-800/50 rounded-xl">
                        <p class="text-dark-400 text-xs mb-1">Öncelik</p>
                        <p class="text-sm font-medium {{ $task->priority === 'urgent' ? 'text-red-400' : ($task->priority === 'high' ? 'text-orange-400' : ($task->priority === 'medium' ? 'text-yellow-400' : 'text-dark-300')) }}">
                            {{ ['low'=>'Düşük','medium'=>'Orta','high'=>'Yüksek','urgent'=>'Acil'][$task->priority] ?? ucfirst($task->priority) }}
                        </p>
                    </div>
                    <div class="p-4 bg-dark-800/50 rounded-xl">
                        <p class="text-dark-400 text-xs mb-1">Vade Tarihi</p>
                        <p class="text-white text-sm font-medium">{{ $task->due_date ? \Carbon\Carbon::parse($task->due_date)->format('d.m.Y') : '-' }}</p>
                    </div>
                    <div class="p-4 bg-dark-800/50 rounded-xl">
                        <p class="text-dark-400 text-xs mb-1">Atanan</p>
            ...
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-white mb-4">İlişkili Kayıtlar</h2>
                <div class="space-y-3">
                    @if($task->contact)
                    <a href="{{ route('admin.contacts.show', $task->contact) }}" class="flex items-center gap-3 p-3 bg-dark-800/50 rounded-xl hover:bg-dark-800 transition-colors">
                        ...
                        <div>
                            ...
                            <p class="text-dark-400 text-xs">Kişi</p>
                        </div>
                    </a>
                    @endif
                    @if($task->lead)
                    <a href="{{ route('admin.leads.show', $task->lead) }}" class="flex items-center gap-3 p-3 bg-dark-800/50 rounded-xl hover:bg-dark-800 transition-colors">
                        ...
                        <div>
                            <p class="text-white text-sm font-medium">Lead #{{ $task->lead->id }}</p>
                            <p class="text-dark-400 text-xs">Talep</p>
                        </div>
                    </a>
                    @endif
                </div>
        ...
        <div class="space-y-6">
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6">
                <p class="text-dark-400 text-sm mb-2">Durum</p>
            ...
            <div class="bg-dark-900 border border-dark-700/50 rounded-2xl p-6 space-y-3">
                ...
                <a href="{{ route('admin.tasks.edit', $task) }}" class="block w-full px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-xl transition-colors text-center">Düzenle</a>
                <form action="{{ route('admin.tasks.destroy', $task) }}" method="POST" onsubmit="return confirm('Görevi silmek istediğinize emin misiniz?')">
                    @csrf @method('DELETE')
                    <button type="submit" class="w-full px-4 py-2.5 bg-dark-700 hover:bg-dark-600 text-dark-300 font-medium rounded-xl transition-colors">Sil</button>
                </form>
            </div>
            @endif
        </div>
